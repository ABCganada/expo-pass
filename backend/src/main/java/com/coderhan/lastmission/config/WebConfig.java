...
/**
 * 프론트엔드가 백엔드와 다른 도메인(lastmission-api.coder-han.com)에 있으므로
 * 크로스 오리진 요청을 허용한다.
 *
 * <p>두 도메인 모두 등록가능도메인이 coder-han.com 이라 브라우저 기준 same-site 다.
 * 따라서 auth-core 세션 쿠키(Domain=coder-han.com, SameSite=Lax)는 그대로 전달되지만,
