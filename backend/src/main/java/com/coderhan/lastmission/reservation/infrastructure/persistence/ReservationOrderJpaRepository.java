...
interface ReservationOrderJpaRepository extends JpaRepository<ReservationOrderEntity, String> {
    ...
    // Payment 도메인 정산 매출 계산용 — orderId 필드만 뽑아온다
    @Query("SELECT o.orderId FROM ReservationOrderEntity o WHERE o.eventId = :eventId")
    List<String> findOrderIdByEventId(@Param("eventId") Long eventId);
