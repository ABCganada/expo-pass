...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      const saved = localStorage.getItem("lastmission-admin-sidebar-collapsed");
  ...
  const handleToggleCollapse = () => {
    ...
    localStorage.setItem("lastmission-admin-sidebar-collapsed", String(collapsed));
  };
    ...
    <div className={styles.shell}>
      {authStatus === "loading" || authStatus === "unauthenticated" ? (
        ...
        <>
          ...
          <MessengerWidget currentUserId={user?.id ?? ""} roles={user?.roles ?? []} mode="admin" />
        </>
      )}
    </div>
  );
